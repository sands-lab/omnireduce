...
namespace daiet {
    ...
    void print_packet(struct rte_ether_hdr *eth, uint16_t size) {
        ...
        s << "** Ethernet **" << endl;
        s << "Src: " + mac_to_str(eth->s_addr) << endl;
        s << "Dst: " + mac_to_str(eth->d_addr) << endl;
        uint16_t etherType = rte_be_to_cpu_16(eth->ether_type);
        s << "EtherType: 0x" + to_hex(etherType) << endl;
        ...
        if (etherType == RTE_ETHER_TYPE_IPV4 && size >= idx + sizeof(struct rte_ipv4_hdr)) {
            ...
            s << "** IPv4 **" << endl;
            s << "Version: " << to_string((ip->version_ihl & 0xf0) >> 4) << endl;
            s << "IHL: " << to_string(ip->version_ihl & 0x0f) << endl;
            s << "TTL: " << to_string(ip->time_to_live) << endl;
            s << "TotalLen: " << to_string(rte_be_to_cpu_16(ip->total_length)) << endl;
            s << "Src: 0x" << to_hex(rte_be_to_cpu_32(ip->src_addr)) << endl;
            s << "Dst: 0x" << to_hex(rte_be_to_cpu_32(ip->dst_addr)) << endl;
            s << "Protocol: " << to_string(ip->next_proto_id) << endl;
            ...
            if (ip->next_proto_id == IPPROTO_UDP && size >= idx + sizeof(struct rte_udp_hdr)) {
                ...
                s << "** UDP **" << endl;
                s << "Src: " << to_string(rte_be_to_cpu_16(udp->src_port)) << endl;
                s << "Dst: " << to_string(rte_be_to_cpu_16(udp->dst_port)) << endl;
                s << "Len: " << to_string(rte_be_to_cpu_16(udp->dgram_len)) << endl;

                if (size >= idx + sizeof(struct daiet_hdr)) {

                    idx += sizeof(struct daiet_hdr);
                    struct daiet_hdr* daiet = (struct daiet_hdr *) (udp + 1);
                    uint16_t pool_index = rte_be_to_cpu_16(daiet->pool_index);

                    s << "** DAIET **" << endl;
                    s << "TSI: " << to_string(daiet->tsi) << endl;
                    s << "PoolIndex: " << to_string(pool_index & 0x7FFF) << 
                             " - " << to_string((pool_index & 0x8000)>>15) << endl;
