...
if args.cuda:
    ...
    cuda_device = torch.device('cuda', hvd.local_rank() % hvd.size())
...
def benchmark_step():
    ...
    data = torch.rand(args.batch_size, 3, 224, 224, device=cuda_device)
    target = torch.randint(0, 1000, (args.batch_size,), device=cuda_device)
    data_index += 1
    optimizer.zero_grad()
    output = model(data)
